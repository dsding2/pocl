...
void
pocl_ispc_init_device_ops(struct pocl_device_ops *ops)
{
  ops->device_name = "cpu-minimal";

  ops->probe = pocl_ispc_probe;
  ops->uninit = pocl_ispc_uninit;
  ops->reinit = pocl_ispc_reinit;
  ops->init = pocl_ispc_init;
  ...
  ops->run = pocl_ispc_run;
  ops->run_native = pocl_ispc_run_native;
  ...
  ops->free_program = pocl_ispc_free_program;
  ...
  ops->compile_kernel = pocl_ispc_compile_kernel;
  ...
  ops->join = pocl_ispc_join;
  ops->submit = pocl_ispc_submit;
  ops->broadcast = pocl_broadcast;
  ops->notify = pocl_ispc_notify;
  ops->flush = pocl_ispc_flush;
  ops->build_hash = pocl_cpu_build_hash;
  ops->compute_local_size = pocl_default_local_size_optimizer;

  ops->get_device_info_ext = pocl_ispc_get_device_info_ext;
  ops->get_subgroup_info_ext = pocl_ispc_get_subgroup_info_ext;
  ops->set_kernel_exec_info_ext = pocl_ispc_set_kernel_exec_info_ext;
  ops->get_synchronized_timestamps = pocl_driver_get_synchronized_timestamps;

  ops->svm_free = pocl_ispc_svm_free;
  ops->svm_alloc = pocl_ispc_svm_alloc;
  ops->usm_alloc = pocl_ispc_usm_alloc;
  ops->usm_free = pocl_ispc_usm_free;
  ...
  ops->create_kernel = pocl_ispc_create_kernel;
  ops->free_kernel = pocl_ispc_free_kernel;
  ops->create_sampler = NULL;
  ops->free_sampler = NULL;
  ops->copy_image_rect = pocl_ispc_copy_image_rect;
  ops->write_image_rect = pocl_ispc_write_image_rect;
  ops->read_image_rect = pocl_ispc_read_image_rect;
  ops->map_image = pocl_ispc_map_image;
  ops->unmap_image = pocl_ispc_unmap_image;
  ops->fill_image = pocl_ispc_fill_image;
}

unsigned int
pocl_ispc_probe(struct pocl_device_ops *ops)
{
  ...
  if (env_count <= 0)
    env_count = pocl_device_get_env_count("ispc");
...
cl_int
pocl_ispc_init (unsigned j, cl_device_id device, const char* parameters)
{
  pocl_basic_data_t *d;
...
void
pocl_ispc_run (void *data, _cl_command_node *cmd)
{
...
void
pocl_ispc_run_native (void *data, _cl_command_node *cmd)
{
...
cl_int
pocl_ispc_uninit (unsigned j, cl_device_id device)
{
...
cl_int
pocl_ispc_reinit (unsigned j, cl_device_id device, const char *parameters)
{
  pocl_basic_data_t *d
...
static void
ispc_command_scheduler (pocl_basic_data_t *d)
{
...
void
pocl_ispc_submit (_cl_command_node *node, cl_command_queue cq)
{
  ...
  POCL_UNLOCK_OBJ (node->sync.event.event);
  ispc_command_scheduler (d);
  POCL_UNLOCK (d->cq_lock);

  return;
}

void pocl_ispc_flush (cl_device_id device, cl_command_queue cq)
{
  pocl_basic_data_t *d = (pocl_basic_data_t *)device->data;
...
void
pocl_ispc_join (cl_device_id device, cl_command_queue cq)
{
  pocl_basic_data_t *d = (pocl_basic_data_t *)device->data;
...
void
pocl_ispc_notify (cl_device_id device, cl_event event, cl_event finished)
{
  pocl_basic_data_t *d = (pocl_basic_data_t *)device->data;
...
int
pocl_ispc_compile_kernel (_cl_command_node *cmd,
                           cl_kernel kernel,
                           cl_device_id device,
                           int specialize)
{
...
int
pocl_ispc_free_program (cl_device_id device, cl_program program,
                          unsigned dev_i)
{
...
cl_int pocl_ispc_copy_image_rect( void *data,
                                   cl_mem src_image,
                                   cl_mem dst_image,
...
cl_int pocl_ispc_write_image_rect (  void *data,
                                      cl_mem dst_image,
                                      pocl_mem_identifier *dst_mem_id,
...
cl_int pocl_ispc_read_image_rect(  void *data,
                                    cl_mem src_image,
                                    pocl_mem_identifier *src_mem_id,
...
cl_int pocl_ispc_map_image (void *data,
                             ...
                             mem_mapping_t *map)
{
  ...
  if (map->host_ptr != ((char *)mem_id->mem_ptr + map->offset))
    {
      pocl_ispc_read_image_rect (data, src_image, mem_id, map->host_ptr,
...
cl_int pocl_ispc_unmap_image(void *data,
                              pocl_mem_identifier *mem_id,
                              cl_mem dst_image,
                              mem_mapping_t *map)
{
  ...
  if (map->host_ptr != ((char *)mem_id->mem_ptr + map->offset))
    {
      pocl_ispc_write_image_rect (data, dst_image, mem_id, map->host_ptr,
                                   NULL, map->origin, map->region,
                                   map->row_pitch, map->slice_pitch, 0);
    }
  return CL_SUCCESS;
...
cl_int
pocl_ispc_fill_image (void *data, cl_mem image,
...
void
pocl_ispc_svm_free (cl_device_id dev, void *svm_ptr)
{
...
void *
pocl_ispc_svm_alloc (cl_device_id dev, cl_svm_mem_flags flags, size_t size)
...
void *
pocl_ispc_usm_alloc (cl_device_id dev, unsigned alloc_type,
...
void
pocl_ispc_usm_free (cl_device_id dev, void *usm_ptr)
{
  pocl_aligned_free (usm_ptr);
}

cl_int
pocl_ispc_get_device_info_ext (cl_device_id device, cl_device_info param_name,
...
cl_int
pocl_ispc_get_subgroup_info_ext (cl_device_id device,
                                  cl_kernel kernel,
...
cl_int
pocl_ispc_set_kernel_exec_info_ext (cl_device_id dev,
                                     unsigned program_device_i,
...
int
pocl_ispc_create_kernel (cl_device_id device,
                          cl_program p,
                          cl_kernel k,
                          unsigned device_i)
{
...
int
pocl_ispc_free_kernel (cl_device_id device,
                        cl_program p,
                        cl_kernel k,
                        unsigned device_i)
{
